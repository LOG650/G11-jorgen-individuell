...
- **Valuta:** Kildedata er denominert i **EUR** (cockpit-rapportene har EUR-kolonneoverskrifter; fakturabeløpene i `costs_clean.csv` ligger i EUR-størrelses­orden). Modellen er trent og rapporteres i EUR. Frontend konverterer til NOK eller DKK ved API-grensa via `EXCHANGE_RATES_FROM_EUR` (mai 2026: EUR/NOK ≈ 11,50, EUR/DKK = 7,46). Konstantene oppdateres mot ECBs referansekurs ved behov og er en ekstra usikkerhets­kilde for NOK-tall (jf. § 9.5). Inflasjonsjustering er ikke gjennomført.
...
**Simuleringsmetode (overordnet).** Data for 2020–2024 ble generert på transaksjons­nivå ankret i tre kilder: (i) reelle 2025-fakturaer fra SDK Shipping — som ga distribusjons­karakteristikker for fakturabeløp per (kategori, størrelse), tjeneste­miks per havn og sesong­profiler, (ii) reelle årlige cockpit-aggregater (`cockpit_clean.csv` inneholder reelle 2020–2025-omsetnings­tall per kategori og år) — som styrer årlig volum og service­fordeling, og (iii) `PORT_TEMPLATES` for havne­strukturen. Yacht-IDer ble bevart, slik at de 17 yachters reelle spesifikasjoner (GT, LOA, fuel-konsum) er knyttet til de simulerte transaksjonene. Sammenligning mot reell 2025 viser kvalitativt sammenlignbare havne- og kategori­fordelinger, samt log-normal beløps­karakteristikk (P95/P50 ≈ 12 i begge). Eksplisitt sesongdynamikk og inflasjons­drift 2020–2024 er ikke modellert — dette er en kjent kilde til simulert→reell-degraderingen i § 9.4 S1 og § 9.6 trussel (iii).
...
Brukertesten avdekket at estimatene fremsto ca. 10× lavere enn faktiske NOK-beløp — en enhets­feil der kildedataene er EUR, men v0.2 merket output NOK (forholdstallet 11,5 = EUR/NOK-kursen). Tiltak: API tar nå et `currency`-felt og konverterer via `EXCHANGE_RATES_FROM_EUR`; frontend har valuta­velger.

#### 9.5.2 Mandatory pilotage modelleres som sannsynlig, ikke obligatorisk

For yachter med LOA > 70 m er los­tjeneste lovpålagt, men `PORT_TEMPLATES` har los som sannsynlighets­vektet linje (17–73 %) på tvers av *alle* størrelser — modellen under­vekter systematisk los­kostnad for Stor-yachter. Dette er gapet § 1.5 punkt (iii) påpeker. Mønsteret er kjent fra havne­effektivitets­litteraturen (Jahangard et al., 2025). Tiltak: API krever nå `pilot_cost` når `loskrav = "Ja"`. Riktig modellfix er størrelse-betingede sannsynligheter (videre arbeid punkt 8).
...
v0.3 hadde valgfri `provisioning_override`; den ble fjernet i v0.4 (commit `e24655a`) på metodisk grunnlag — en override som erstatter prediksjonen med et magefølelses­tall undergraver verktøyets formål. For bunkers er overriden forsvarlig fordi den substitueres med en *deterministisk fysisk formel*. Provisions-feilen skal lukkes via labels + re-trening, ikke maskeres via UI.
